...
class MovieLayer(tf.keras.layers.Layer):
    def __init__(self, features):
        ...
        self.movieID_embedding = tf.keras.layers.Embedding(movie_id_max, embed_dim, input_length=1,
                                                           name='movie_id_embed_layer')

        self.movieID_fc_layer = tf.keras.layers.Dense(embed_dim, name="movie_id_fc_layer", activation='relu')

        # 添加genres 嵌入层
        self.genres_embedding = tf.keras.layers.Embedding(genres_max, embed_dim, input_length=20,
                                                name='genres_embed_layer')
        self.genres_embedding_reduce = tf.keras.layers.Lambda(lambda layer: tf.reduce_sum(layer, axis=1, keepdims=True))

        self.genres_fc_layer = tf.keras.layers.Dense(embed_dim, name="genres_fc_layer", activation='relu')


        self.titles_layer = MovieCnnLayer(features)

        self.dense = tf.keras.layers.Dense(200, activation='tanh')

    def call(self, inputs, **kwargs):
        movie_id, movie_genres, movie_titles = inputs

        mid_embed_layer = self.movieID_embedding(movie_id)
        movie_id_fc_layer = self.movieID_fc_layer(mid_embed_layer)

        genres_embedding = self.genres_embedding(movie_genres)
        genres_embed_layer = self.genres_embedding_reduce(genres_embedding)
        genres_fc_layer = self.genres_fc_layer(genres_embed_layer)
        dropout_layer = self.titles_layer(movie_titles)
        movie_combine_layer = tf.concat([movie_id_fc_layer, genres_fc_layer, dropout_layer],2)

        output =  self.dense(movie_combine_layer)  # (?, 1, 200)
        return output
...
class DeepLearningNetwork(tf.keras.Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        movie_out = self.movie_layer([movie_id, movie_genres, movie_titles])
    ...
    def summary(self, line_length=None, positions=None, print_fn=None):
        ...
        tf.keras.Model(inputs=[uid,movie_id,movie_titles,movie_genres],
                       outputs = self.call([uid,movie_id,movie_titles,movie_genres])).summary()
...
if __name__ == '__main__':
    ...
    targets_values = targets_values[:1000]
